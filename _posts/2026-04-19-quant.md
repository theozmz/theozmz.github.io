...
* Given the following data points for actual values ($$Y$$) and predicted values ($$\hat{Y}$$) from a linear regression model: What is the R-squared value? (round to 2 decimals and choose the closest answer)
  * A) 0.85
  * B) 0.70
  * C) 0.90
  * D) 0.75
