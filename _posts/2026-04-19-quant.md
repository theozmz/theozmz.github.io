...
* Which of the following scenarios exemplifies the use of supervised learning in algorithmic trading? **C**
...
* Consider a trading scenario where an agent can take three actions: Buy (1), Sell (2), or Hold (0). After taking the action to Buy, the agent receives a reward of $50. The Q-learning update formula is as follows: $$Q(s, a) \leftarrow Q(s, a) + \alpha\left[r + \gamma \max_{a'} \left(Q(s', a')\right) - Q(s, a)\right]$$. If the current Q-value for the state-action pair $$Q(1,1)$$ (State 1, Action Buy) is 10, the learning rate $$\alpha$$ is 0.1, the discount factor $$\gamma$$ is 0.9, and the maximum Q-value for the next state $$s'$$ after the action is 30, what will be the updated Q-value for $$Q(1,1)$$? **C**
  * A) 10.1
  * B) 14.8
  * **C) 16.7**
  * D) 21.4
...
$$Q(1,1)=10+0.1\times(50+0.9\times30-10)=16.7$$

* Which of the following is a significant limitation of Q-learning when applied to trading environments? **B**
...
* If a new stock has a return of (0.12, 0.08), based on Euclidean distance measure, which cluster will this stock most likely be assigned to? **B**
  * A) Cluster 1
  * **B) Cluster 2**
  * C) Cluster 3
...
* Which of the following statements about Fund A and Fund B is TRUE? **E**
...
* Basic Information
  * A: $$\mu=0.12,\sigma=0.1,\beta=1.2,\sigma_{Sortino}=0.06,\mu_{Benchmark}=0.1$$
  * B: $$\mu=0.08,\sigma=0.04,\beta=0.5,\sigma_{Sortino}=0.02,\mu_{Benchmark}=0.06$$
  * Risk-free rate: 0.03
* Sharpe Ratio
  * A: $$\frac{\mu-RiskFreeRate}{\sigma}=\frac{0.12-0.03}{0.1}=0.9$$
  * B: $$\frac{\mu-RiskFreeRate}{\sigma}=\frac{0.08-0.03}{0.04}=1.25$$
* Sortino Ratio
  * A: $$\frac{\mu-RiskFreeRate}{\sigma_{Sortino}}=\frac{0.12-0.03}{0.06}=1.5$$
  * B: $$\frac{\mu-RiskFreeRate}{\sigma_{Sortino}}=\frac{0.08-0.03}{0.02}=2.5$$
* Treynor Ratio
  * A: $$\frac{\mu-RiskFreeRate}{\beta}=\frac{0.12-0.03}{1.2}=0.075$$
  * B: $$\frac{\mu-RiskFreeRate}{\beta}=\frac{0.08-0.03}{0.5}=0.1$$
* Jensen's Alpha
  * A: $$\mu-(RiskFreeRate+\beta(\mu_{Benchmark}-RiskFreeRate))=0.12-(0.03+1.2\times(0.1-0.03))=0.006$$
  * B: $$\mu-(RiskFreeRate+\beta(\mu_{Benchmark}-RiskFreeRate))=0.08-(0.03+0.5\times(0.06-0.03))=0.035$$
* Information Ratio: $$\sigma_{Benchmark}$$不知道


## 👉Takeaway👈

1. Spread
  * $$\frac{2\times(Ask-Bid)}{Ask+Bid}$$
2. EMA
  * $$EMA_t=w\times P_t+(1-w)\times EMA_{t-1}$$
3. EWMA
  * $$Return_t=\frac{P_t-P_{t-1}}{P_{t-1}}$$
  * $$\sigma_t^2=(1-w)\times R_t^2+w\times \sigma_{t-1}^2$$
...
7. Hurst Exponent half-life
  * $$t=\frac{ln(2)}{\kappa}$$
8. ARCH / GARCH
  * 用$$\sigma$$替换a
  * Unconditional variance: 
    * $$Var(a_t)_{ARCH}=\frac{\alpha_0}{1-\alpha_1-\dots-\alpha_m}$$
    * $$Var(a_t)_{GARCH}=\frac{\alpha_0}{1-\alpha_1-\beta_1-\dots-\alpha_m-\beta_m}$$
9. VaR
  * $$VaR=\mu-z\sigma$$
  * $$RelativeVaR=z\sigma$$
10. Kelly Criterion
  * $$f^*=\frac{bp-q}{b}$$
11. Portfolio
  * 先从单个return算portfolio的return($$\mu$$)，再从return算portfolio的volatility($$\sigma$$)
  * $$R_p=w_AR_A+w_BR_B$$
...
13. Tangency Portfolio
  * 超额收益 $$R^e=\mu-RiskFreeRisk$$
  * $$w_A=\frac{R_A^e\sigma_B^2-R_B^e\sigma_{AB}}{R_B^e\sigma_A^2+R_A^e\sigma_B^2-(R_A^e+R_B^e)\sigma_{AB}}$$
14. Market Portfolio
  * $$w_i^*=\frac{P_iS_i}{\sum_{j=1}^N P_jS_j}
15. CAPM
  * $$R_p=R_f+\beta(R_m-R_f)$$
16. Sharpe Ratio
  * $$SR=\frac{\mu-RiskFreeRate}{\sigma}$$
  * ⚠️⚠️注意这里是标准偏差，分母除以$$n-1$$⚠️⚠️
  * ⚠️⚠️注意月度收益要年化，需要乘$$\sqrt{12}$$⚠️⚠️
17. Sortino Ratio
  * Ratio计算公式和Sharpe一样，标准偏差需要用到负收益计算
...
18. Treynor Ratio
  ...
  * 分母是贝塔，一般会给出
19. Jensen’s Alpha
  * $$\alpha_{portfolio}=\mu_{portfolio}-[RiskFreeRate+\beta(\mu_{market}-RiskFreeRate)]$$
  * 中括号里的是CAPM
20. Information Ratio
  * $$\Rightarrow IR = \frac{\alpha_p}{\tau_p}$$
  * 分子是Jensen’s Alpha
  * $$\tau_p=\sqrt{\sigma_p^2-\beta_p^2 \sigma_M^2}$$
21. Maximum Drawdown
  * 最大值动态更新
  * 计算所有由最大值到当前值的drawdown，取所有drawdown最大值
22. Calmar Ratio
  * $$Calmar\ Ratio = \frac{Annualized\ Return}{\|Max.\ Drawdown\|}$$
23. TWAP
  * $$Round(\frac{TotalOrderSize\times Interval}{ExecutionDuration}, 0)$$
24. VWAP
